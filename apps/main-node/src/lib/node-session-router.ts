...
import type { NodeSessionWorkQueue } from "./node-session-work-queue.js";
...
export class NodeSessionRouter implements SessionRouter {
  ...
  async exec(
    ...
    // Reach into the registry for the active sandbox. Adapters historically
    // returned either "exit=N\n<output>" or "<output>\n[exit N]"; normalize
    // both shapes for the HTTP exec route.
    const row = await this.deps.sql
      .prepare(`SELECT tenant_id FROM sessions WHERE id = ?`)
      .bind(sessionId)
      .first<{ tenant_id: string }>();
    ...
    if (prefix) {
      return {
        exit_code: parseInt(prefix[1], 10),
        output: prefix[2],
        truncated: false,
      };
    }
    const suffix = /(?:^|\n)\[exit (?:exit=)?(-?\d+)\]\s*$/.exec(out);
    if (!suffix) return { exit_code: 0, output: out, truncated: false };
    return {
      exit_code: parseInt(suffix[1], 10),
      output: out.slice(0, suffix.index).replace(/\s+$/, ""),
      truncated: false,
    };
  }
