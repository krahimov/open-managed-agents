 ...
 *                 │  or x-api-key: <server-side project key>
 *                 ▼
 *           upstream MCP server
 *
...
export interface ProxyTarget {
  ...
  upstreamUrl: string;
  /** Token-like secret used for refresh bookkeeping and legacy bearer callers. */
  upstreamToken: string;
  /** Exact upstream auth header. Defaults to Authorization: Bearer <upstreamToken>. */
  upstreamAuthHeader?: {
    name: string;
    value: string;
  };
...
function exactBearerHeader(token: string): NonNullable<ProxyTarget["upstreamAuthHeader"]> {
  return { name: "authorization", value: `Bearer ${token}` };
}

function resolveComposioApiKey(
  env: Env,
  auth: { api_key?: string; api_key_env?: string },
): string | null {
  if (typeof auth.api_key === "string" && auth.api_key.length > 0) return auth.api_key;
  const envName = auth.api_key_env || "COMPOSIO_API_KEY";
  const raw = (env as unknown as Record<string, unknown>)[envName];
...
function isComposioMcpUrl(raw: string): boolean {
  try {
    ...
    if (url.hostname === "connect.composio.dev" && url.pathname.replace(/\/+$/, "") === "/mcp") {
      return true;
    }
    return (
  ...
  for (const g of grouped) {
    for (const c of g.credentials) {
      const auth = (c as unknown as CredentialConfig).auth as
        ...
        | undefined;
      if (!auth || !credentialMatchesServerUrl(auth, server.url)) continue;
      if (auth.type === "composio_mcp") {
        const apiKey = resolveComposioApiKey(env, auth);
        if (!apiKey) continue;
        return {
...
export async function forwardToUpstream(
  ...
  for (const h of SENSITIVE_UPSTREAM_AUTH_HEADERS) upstreamHeaders.delete(h);
  const authHeader = target.upstreamAuthHeader ?? exactBearerHeader(target.upstreamToken);
...
export async function forwardWithRefresh(
  ...
  const retried = await forwardToUpstream(
    { ...target, upstreamToken: fresh, upstreamAuthHeader: exactBearerHeader(fresh) },
    method,
    inboundHeaders,
    body,
  );
