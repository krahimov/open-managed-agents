...
/**
 * The default web_search tool, standalone so non-agent surfaces (the setup
 * harness) can offer search without the full buildTools dependency set.
 * Backend picked at build time:
 *   - TAVILY_API_KEY present → Tavily API (reliable, rate-limit-free at
 *     normal volumes). One env var upgrades every consumer's default
 ...
 */
export function buildDefaultWebSearchTool(env?: { TAVILY_API_KEY?: string }) {
...
export async function buildTools(
  ...
  if (enabled.has("bash")) {
    tools.bash = tool({
      description:
        ...
        "the read tool across turns. Do not use bash CLIs to access OAuth-connected " +
        "external services such as Google Drive, Gmail, Notion, Slack, GitHub, or Linear; " +
        "use the corresponding MCP tools because provider credentials are injected there, " +
        "not into the shell environment.",
      inputSchema: z.object({
  ...
  if (toolTypes.has("web_search_20250305")) {
    ...
    tools.web_search = buildDefaultWebSearchTool(env);
  }
