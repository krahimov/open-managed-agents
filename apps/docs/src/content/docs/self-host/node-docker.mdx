...
# Local smoke test only: bypass auth so the curl commands below work.
# Never use this on a shared or Internet-facing deployment.
# AUTH_DISABLED=1
#
# Configure one model provider. The example turn below uses OpenAI:
# OPENAI_API_KEY=<disposable low-limit key>
# OPENAI_MODEL=gpt-5.4-mini
# Or set ANTHROPIC_API_KEY and use claude-sonnet-4-6 below.
...
Drive the harness end-to-end. These requests intentionally omit authentication because the local smoke-test configuration above sets `AUTH_DISABLED=1`. If auth is enabled, sign in through the Console and add a real tenant `x-api-key` to every request.

```bash
MODEL=gpt-5.4-mini
AID=$(curl --fail --silent -X POST localhost:8787/v1/agents -H 'content-type: application/json' \
  -d "{\"name\":\"hello\",\"model\":\"$MODEL\",\"tools\":[{\"type\":\"agent_toolset_20260401\"}]}" \
  | jq -r .id)
test -n "$AID" && test "$AID" != null

SID=$(curl --fail --silent -X POST localhost:8787/v1/sessions -H 'content-type: application/json' \
  -d "{\"agent\":\"$AID\"}" | jq -r .id)
...
| **GitHub OAuth** | Team SSO | `GITHUB_AUTH_CLIENT_ID` + `GITHUB_AUTH_CLIENT_SECRET` in `.env`. Console renders GitHub button automatically. |
| **Better Auth Infrastructure** | Optional hosted dashboard/audit logs | `BETTER_AUTH_API_KEY` in `.env`. `BETTER_AUTH_API_URL`/`BETTER_AUTH_KV_URL` usually stay unset unless Better Auth gives custom endpoints. |
...
| `daytona` | Production / untrusted code | `SANDBOX_PROVIDER=daytona DAYTONA_API_KEY=...` (+ optional `DAYTONA_API_URL`, `SANDBOX_IMAGE`, `DAYTONA_BOOTSTRAP_TOOLS`). Daytona-managed Linux VMs, one per session. Defaults to a Node 22 Debian image plus a Python/Git/curl/jq/ripgrep/build-tools bootstrap in `/workspace`. |
...
<Aside type="caution" title="Remote memory stores require shared object storage">
  Daytona, E2B, and BoxRun cannot use the Node host's local memory-blob directory. Before attaching a memory store to a remote-sandbox session, configure the `MEMORY_S3_*` variables for an S3-compatible store (AWS, R2, OSS, or MinIO). Missing shared storage can surface as a stalled or failed turn rather than an obvious mount error. Local `subprocess` sessions use `./data/memory-blobs` and do not need S3.
</Aside>
