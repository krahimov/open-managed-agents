...
## Custom OAuth (Console sign-in)

This one IS deploy-wide — Google or GitHub sign-in adds an SSO path to the Console alongside email + password / email-OTP.
...
2. Set the authorized callback URL:
   - Google: `https://app.yourdomain.com/auth/callback/google`
   - GitHub: `https://app.yourdomain.com/auth/callback/github`
...
3. Capture client ID + secret. Set:
   ...
   Set only the provider you intend to enable. `GITHUB_CLIENT_ID` /
   `GITHUB_CLIENT_SECRET` are for Console sign-in; `GITHUB_OAUTH_*` is a
   separate downstream integration/vault flow.
