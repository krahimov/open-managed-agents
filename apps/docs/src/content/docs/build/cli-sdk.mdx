...
For an interactive machine or coding agent, sign in through the browser
handoff:
...
If no local credentials are stored, the CLI opens Harness Studio in the
browser, waits for the loopback callback, and stores tenant-scoped credentials
under your local `oma` config directory. Future CLI calls from Codex, Claude
Code, or a shell can reuse those credentials.

For self-hosters or staging deployments, point the sign-in at your deployment:

```bash
oma auth ensure --base-url "https://app.openma.dev"
```

For non-interactive scripts, you can still provide an API key explicitly:
...
Set `OMA_AUTH_AUTO_LOGIN=0` when a script should fail fast instead of opening
the browser sign-in fallback.
...
# Verify platform auth, opening browser sign-in if needed
oma auth ensure

# Preview how a JSON agent/project file will be applied
oma agents plan -f ./my-agent.json

# Create or update an agent from a JSON file
oma agents apply -f ./my-agent.json

# Skip browser/OAuth app setup in CI or mock tests
oma agents apply -f ./my-agent.json --no-auth

# Force-create a new agent from a JSON file
