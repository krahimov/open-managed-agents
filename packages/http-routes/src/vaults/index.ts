...
export function buildVaultRoutes(deps: VaultRoutesDeps) {
  ...
  app.post("/:id/credentials/composio_tool_router_session", async (c) => {
    ...
    try {
      const session = await createComposioToolRouterSession(deps.composio, sessionPayload);
  ...
  app.get("/:id/credentials/composio_toolkits", async (c) => {
    ...
    try {
      const catalog = await listComposioToolkits(deps.composio, {
  ...
  app.get("/:id/credentials/composio_accounts", async (c) => {
    ...
    try {
      const accounts = await listComposioConnectedAccounts(deps.composio, {
        userId,
  ...
  app.post("/:id/credentials/composio_accounts/link", async (c) => {
    ...
    try {
      const link = await createComposioConnectedAccountLink(deps.composio, {
        userId,
