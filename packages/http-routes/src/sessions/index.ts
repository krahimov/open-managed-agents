...
function defaultEnvironmentIdFromAgent(agent: AgentConfig): string | undefined {
  const value = agent.metadata?.default_environment_id;
  return typeof value === "string" && value ? value : undefined;
}

function defaultVaultIdsFromAgent(agent: AgentConfig): string[] {
  const value = agent.metadata?.default_vault_ids;
...
export function buildSessionRoutes(deps: SessionRoutesDeps) {
  ...
  app.post("/", async (c) => {
    ...
    let envId = body.environment_id ?? wrappedEnv ?? defaultEnvironmentIdFromAgent(agentRow);
    if (!envId) {
    ...
    const vaultIds = uniqueStrings([
      ...defaultVaultIdsFromAgent(agentRow),
      ...(body.vault_ids ?? []),
    ]);
