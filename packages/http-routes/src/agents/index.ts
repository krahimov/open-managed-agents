...
function formatAgent(agent: AgentConfig) {
  ...
  if (typeof metadata.default_environment_id === "string") {
    oma.default_environment_id = metadata.default_environment_id;
  }
  if (
    Array.isArray(metadata.default_vault_ids) &&
    metadata.default_vault_ids.every((id) => typeof id === "string")
  ) {
    oma.default_vault_ids = metadata.default_vault_ids;
  }
