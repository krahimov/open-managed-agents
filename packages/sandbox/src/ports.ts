...
export interface SandboxFactoryContext {
   ...
   *  (Daytona/E2B) use provider-specific S3/R2 mount or sync paths. */
  memoryRoot?: string;
 ...
 *  this shape (S3/R2 bucket config). */
export function readS3MemoryBucket(env: SandboxFactoryEnv):
  | { endpoint: string; accessKey: string; secretKey: string; bucketName: string; region?: string }
  | undefined {
  ...
  const r = env.MEMORY_S3_REGION;
  return e && a && s && b
    ? { endpoint: e, accessKey: a, secretKey: s, bucketName: b, ...(r ? { region: r } : {}) }
    : undefined;
}
