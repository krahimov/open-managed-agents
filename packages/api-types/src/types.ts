...
export interface EnvironmentConfig {
  ...
  config: {
    ...
    /** Self-host runtime hints. The hosted Anthropic surface treats
     *  environments as provider-owned; Open Managed Agents uses this block
     *  to map an approved environment to a concrete sandbox backend. */
    sandbox?: {
      provider?: string;
      image?: string;
      workdir?: string;
     ...
     *  these mounts for a specific session. */
    memory?: {
      stores?: Array<{
        id?: string;
        store_id?: string;
    ...
    /** Environment resources that should be attached by default. */
    resources?: {
      outputs?: boolean;
      memory_stores?: Array<{
        id?: string;
...
export interface CredentialAuth {
  type: "mcp_oauth" | "static_bearer" | "cap_cli" | "composio_mcp";
  // mcp_oauth / static_bearer / composio_mcp: match by MCP server URL
  mcp_server_url?: string;
  // mcp_oauth fields
  access_token?: string;
  ...
  token?: string;
  // composio_mcp: Composio MCP uses x-api-key, while provider OAuth tokens
  // stay inside Composio. Prefer api_key_env="COMPOSIO_API_KEY"; api_key is
  // accepted for tenant-specific deployments and is stripped from API reads.
  api_key?: string;
  api_key_env?: string;
  composio_user_id?: string;
  composio_session_id?: string;
  composio_toolkits?: string[];
  composio_connected_account_ids?: string[];
