...
import { forwardToUpstream, forwardWithRefresh } from "../../apps/main/src/routes/mcp-proxy";
...
describe("forwardWithRefresh — OAuth refresh on 401", () => {
  ...
  it("injects Composio MCP x-api-key without forwarding caller auth headers", async () => {
    mock.restore();
    mock = installFetchMock([
      (call) => {
        expect(call.headers.get("x-api-key")).toBe("cmp_project_key");
    ...
    const res = await forwardToUpstream(
      {
        upstreamUrl: "https://connect.composio.dev/mcp",
